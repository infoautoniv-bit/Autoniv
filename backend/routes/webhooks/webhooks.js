...
router.post('/vapi', webhookLimiter, async (req, res) => {
  ...
  // Idempotency Check: Skip processing if this webhook event was already handled
  const eventId = req.headers['x-vapi-event-id'] || (callData?.id ? `${callData.id}_${mappedType}` : null);
  if (eventId && isDuplicateEvent(eventId)) {
    log.info('webhook_duplicate_skipped', { eventId, eventType });
...
async function handleTranscript(call, transcript) {
  ...
  const rawTranscript = typeof transcript === 'string'
    ? transcript.slice(0, 200000)
    : JSON.stringify(transcript).slice(0, 200000);

  const safeTranscript = sanitizeText(rawTranscript);
...
<Response>
  ...
  const userSpeech = sanitizeText(rawUserSpeech);

  if (rawUserSpeech && containsAbuse(rawUserSpeech)) {
    log.warn('incoming_call_abusive_speech_detected', { callSid });
  }
