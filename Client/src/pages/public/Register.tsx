          ...
          {showOtp ? (
            <form onSubmit={handleVerifyOtp} className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="block text-sm font-semibold text-slate-700 text-center">Verification Code</label>
                <div className="flex justify-between gap-2 my-4">
                  {otp.map((data, index) => (
                    <input
                      ...
                      className="w-12 h-12 text-center text-xl font-bold bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between text-sm">
                <button
                  ...
                  disabled={timer > 0}
                  className="text-[var(--indigo)] hover:text-[var(--secondary)] font-bold transition-colors disabled:opacity-50"
                >
              ...
              <button
                ...
                className="w-full py-3.5 btn-cta text-white rounded-xl font-bold transition-all disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer"
              >
                {loading ? (
                  <>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                <label htmlFor="reg-name" className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Full Name</label>
                <input
                  id="reg-name"
                  type="text"
                  value={name}
                  ...
                  required
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                />
              </div>

              <div className="space-y-1">
                <label htmlFor="reg-email" className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Email</label>
                <input
                  id="reg-email"
                  type="email"
                  value={email}
              ...
              <div className="space-y-1">
                <label htmlFor="reg-phone" className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Phone Number</label>
                <input
                  id="reg-phone"
                  type="tel"
                  value={phoneNumber}
              ...
              <div className="space-y-1">
                <label htmlFor="reg-company" className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Company (optional)</label>
                <input
                  id="reg-company"
                  type="text"
                  value={company}
              ...
              <div className="space-y-1">
                <label htmlFor="reg-password" className="block text-[10px] font-bold text-slate-600 uppercase tracking-wider">Password</label>
                <div className="relative">
                  <input
                    ...
                    required
                    minLength={6}
                    className="w-full px-4 py-2.5 pr-11 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full py-3.5 btn-cta text-white rounded-xl font-bold transition-all disabled:opacity-50 flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-[var(--indigo)]/10 mt-3"
              >
                {loading ? (
                  <>
                    <svg className="animate-spin w-5 h-5" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
              ...
              <p className="text-center text-sm text-slate-500">
                ...
                <Link to="/login" className="text-[var(--indigo)] hover:text-[var(--secondary)] font-bold transition-colors">
                  Sign in
                </Link>
              </p>
            </form>
