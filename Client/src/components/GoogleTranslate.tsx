...
export function GoogleTranslate() {
  ...
  return (
    <div className="relative inline-block text-left notranslate" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-72 sm:w-80 rounded-2xl bg-white/98 backdrop-blur-xl border border-slate-200/90 shadow-2xl z-[99999] overflow-hidden p-3 transition-all duration-200">
          ...
          <div className="relative mb-2">
            <input
              ...
              className="w-full px-3 py-2 pl-8 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-slate-800 placeholder-slate-400 font-medium"
            />
            <svg
          ...
          <div className="max-h-64 overflow-y-auto space-y-1 pr-1 custom-scrollbar">
            {filteredLanguages.length > 0 ? (
              ...
              <div className="py-4 text-center text-xs text-slate-400 font-medium">No language found</div>
            )}
          </div>
        </div>
      )}
